const intcode2 = (stateCarrier, queue) => {
  let converted = stateCarrier.parsedInput.slice();
  
  let i = stateCarrier.instructionPointer;
  let opcode = converted[i];

  let systemIds = stateCarrier.queue.concat(queue);
  let finalCode = stateCarrier.finalCode;
  ...
  while (opcode !== 99) {
    ...
    switch (parameterizedOpcode) {
      ...
      case 3: {
        ...
        converted[finalPosition] = systemIds.shift();
  ...
  return {
    ...
    'parsedInput': converted,
    'finalCode': finalCode,
    'queue': systemIds
  }
}

module.exports = intcode2;